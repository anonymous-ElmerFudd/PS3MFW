...
# Option --patch-lv1-rebug-dispmgr-access: Allow access to all SS services (Needed for ps3dm-utils)
# Option --patch-lv1-rebug-iimgr-access: Allow access to all services of Indi Info Manager
# Option --patch-lv1-rebug-um-extract-pkg: Allow extracting for all package types
# Option --patch-lv1-rebug-um-write-eprom-product-mode: Allow enabling product mode by using Update Manager Write EPROM
# Option --patch-lv1-rebug-sm-del-encdec-key: Allow deleting of all ENCDEC keys
# Option --patch-lv1-rebug-repo-node-lpar: Allow creating/modifying/deleting of repository nodes in any LPAR
# Option --patch-lv1-rebug-ata-region0-access: Allow access to all regions of all storage devices
# Option --patch-lv1-rebug-sysmgr-disable-integrity-check: Disable integrity check in System Manager
# Option --patch-lv1-rebug-gameos-sysmgr-ability: Allow access to all System Manager services of GameOS
# Option --patch-lv1-rebug-otheros-plus-plus: OtherOS++ support

# Type --patch-lv1-rebug-htab-write: boolean
# Type --patch-lv1-rebug-encdec-ioctl-0x85: boolean
# Type --patch-lv1-rebug-dispmgr-access: boolean
# Type --patch-lv1-rebug-iimgr-access: boolean
# Type --patch-lv1-rebug-um-extract-pkg: boolean
# Type --patch-lv1-rebug-um-write-eprom-product-mode: boolean
# Type --patch-lv1-rebug-sm-del-encdec-key: boolean
# Type --patch-lv1-rebug-repo-node-lpar: boolean
# Type --patch-lv1-rebug-ata-region0-access: boolean
# Type --patch-lv1-rebug-sysmgr-disable-integrity-check: boolean
# Type --patch-lv1-rebug-gameos-sysmgr-ability: boolean
# Type --patch-lv1-rebug-otheros-plus-plus: boolean
...
namespace eval ::patch_lv1_for_rebug {
    ...
    proc main { } {
		...
		# modify_core_os NO LONGER SUPPORTED, as core OS files are
		# auto-unpacked/packed at start/finish of MFW
        #::modify_coreos_file $self ::patch_lv1_for_rebug::patch_self
		::modify_self_file $file ::patch_lv1_for_rebug::patch_elf
    }
   
    proc patch_elf {elf} {
	
        if {$::patch_lv1_for_rebug::options(--patch-lv1-rebug-htab-write)} {
        ...
        if {$::patch_lv1_for_rebug::options(--patch-lv1-rebug-encdec-ioctl-0x85)} {
        ...
        if {$::patch_lv1_for_rebug::options(--patch-lv1-rebug-dispmgr-access)} {
        ...
        if {$::patch_lv1_for_rebug::options(--patch-lv1-rebug-iimgr-access)} {
        ...
        if {$::patch_lv1_for_rebug::options(--patch-lv1-rebug-um-extract-pkg)} {
        ...
        if {$::patch_lv1_for_rebug::options(--patch-lv1-rebug-um-write-eprom-product-mode)} {
        ...
        if {$::patch_lv1_for_rebug::options(--patch-lv1-rebug-sm-del-encdec-key)} {
        ...
        if {$::patch_lv1_for_rebug::options(--patch-lv1-rebug-repo-node-lpar)} {
        ...
        if {$::patch_lv1_for_rebug::options(--patch-lv1-rebug-ata-region0-access)} {
            log "Patching System Manager to allow access to all regions of all storage devices"

            set search  "\x98\x1f\x00\x00\x38\x00\x00\x01\xf9\x21\x00\x80\xf8\x01\x00\x78\xf9\x61\x00\x88"
	    append search "\xf9\x41\x00\x90\xf9\x01\x00\x98"
	    set replace "\x60\x00\x00\x00"
        ...
        if {$::patch_lv1_for_rebug::options(--patch-lv1-rebug-sysmgr-disable-integrity-check)} {
        ...
        if {$::patch_lv1_for_rebug::options(--patch-lv1-rebug-gameos-sysmgr-ability)} {
        ...
        if {$::patch_lv1_for_rebug::options(--patch-lv1-rebug-otheros-plus-plus)} {
	    ...
	    append replace "\x3b\xff\xff\xc8\xe8\x1f\x04\x50\xf8\x01\x00\x88\xe8\x1f\x04\x58\xf8\x01\x00\x90"
	    append replace "\xe8\x1f\x04\x60\xf8\x01\x00\x98\xe8\x1f\x04\x68\xf8\x01\x00\xa0\x38\x00\x00\x00"
	    ...
	    append replace "\x3b\x60\x00\x0f\x40\x9e\x03\x08\xe8\x1f\x04\x50\xf8\x01\x00\x88\xe8\x1f\x04\x58"
	    append replace "\xf8\x01\x00\x90\xe8\x1f\x04\x70\xf8\x01\x00\x98\xe8\x1f\x04\x78\xf8\x01\x00\xa0"
	    ...
	    append replace "\x2f\x80\x00\xff\x3b\x60\x00\x0f\x40\x9e\x02\xa0\xe8\x1f\x04\x50\xf8\x01\x00\x88"
	    append replace "\xe8\x1f\x04\x58\xf8\x01\x00\x90\xe8\x1f\x04\x80\xf8\x01\x00\x98\xe8\x1f\x04\x78"
	    ...
	    append replace "\x78\x00\x06\x20\x2f\x80\x00\xff\x3b\x60\x00\x0f\x40\x9e\x02\x38\xe8\x1f\x04\x50"
	    append replace "\xf8\x01\x00\x88\xe8\x1f\x04\x58\xf8\x01\x00\x90\xe8\x1f\x04\x88\xf8\x01\x00\x98"
	    append replace "\xe8\x1f\x04\x78\xf8\x01\x00\xa0\x38\x00\x00\x00\xf8\x01\x00\x78\xf8\x01\x00\x80"
	    ...
	    append replace "\xe8\x01\x00\x78\x78\x00\x06\x20\x2f\x80\x00\xff\x3b\x60\x00\x0f\x40\x9e\x01\xd0"
	    append replace "\xe8\x61\x00\xb0\x38\x80\x00\x00\xeb\x5f\x04\x0c\xeb\x9f\x04\x30\x7c\xba\xe2\x14"
	    append replace "\x38\xc1\x00\xa8\x3b\xc0\x00\x00\x67\xde\x80\x00\x63\xde\x26\xb4\x7f\xc9\x03\xa6"
	    append replace "\x4e\x80\x04\x21\x2f\x83\x00\x00\x7c\x7b\x1b\x78\x40\x9e\x01\x98\xe8\x61\x00\xa8"
	    append replace "\x38\x80\x00\x00\x7f\x85\xe3\x78\x3b\xc0\x00\x00\x67\xde\x80\x00\x63\xde\x02\x78"
	    append replace "\x7f\xc9\x03\xa6\x4e\x80\x04\x21\x38\x7f\x04\x38\x38\x80\x00\x00\x3b\x60\x00\x10"
	    append replace "\x3b\xc0\x00\x00\x67\xde\x80\x01\x63\xde\x3d\x40\x7f\xc9\x03\xa6\x4e\x80\x04\x21"
	    append replace "\x2f\x83\x00\x00\x7c\x7d\x1b\x78\x41\x9c\x01\x30\x7f\xa3\x07\xb4\xe8\x81\x00\xa8"
	    append replace "\x3b\x20\x08\x00\x7f\x25\xcb\x78\x3b\x60\x00\x10\x3b\xc0\x00\x00\x67\xde\x80\x01"
	    append replace "\x63\xde\x3d\xb8\x7f\xc9\x03\xa6\x4e\x80\x04\x21\x7f\xa3\xc8\x00\x40\x9e\x00\xe8"
	    append replace "\x3b\x60\x00\x14\x38\x7f\x04\x14\xe8\x81\x00\xa8\x38\xa0\x00\x10\x3b\xc0\x00\x00"
	    append replace "\x67\xde\x80\x01\x63\xde\x39\xe0\x7f\xc9\x03\xa6\x4e\x80\x04\x21\x2f\xa3\x00\x00"
	    append replace "\x40\x9e\x00\xbc\xe8\xa1\x00\xa8\x83\x25\x00\x10\x2f\x99\x00\x01\x40\x9e\x00\xac"
	    append replace "\xe8\xa1\x00\xa8\x83\x25\x00\x20\x2f\x99\x00\x00\x40\x9e\x00\x9c\xe8\xa1\x00\xa8"
	    append replace "\x83\x25\x02\x00\x2f\x99\x00\x00\x41\x9e\x00\x8c\xe8\xa1\x00\xa8\x83\x25\x00\x24"
	    append replace "\x7f\xb9\xe0\x00\x41\x9d\x00\x7c\x7f\xa3\x07\xb4\xe8\x81\x00\xa8\x7f\x25\xcb\x78"
	    append replace "\x3b\x60\x00\x10\x3b\xc0\x00\x00\x67\xde\x80\x01\x63\xde\x3d\xb8\x7f\xc9\x03\xa6"
	    append replace "\x4e\x80\x04\x21\x7f\xa3\xc8\x00\x40\x9e\x00\x50\x39\x20\x00\x00\x48\x00\x00\x14"
	    ...
	    append replace "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x01\x80\x00\x00"
	    append replace "\x2f\x64\x65\x76\x2f\x72\x66\x6c\x61\x73\x68\x5f\x6c\x78\x00\x00\x00\x00\x00\x00"
	    append replace "\x00\x00\x00\x00\x00\x00\x00\x00\x73\x73\x00\x00\x70\x61\x72\x61\x6d\x00\x00\x00"
	    append replace "\x75\x70\x64\x61\x74\x65\x00\x00\x73\x74\x61\x74\x75\x73\x00\x00\x70\x72\x6f\x64"
	    append replace "\x75\x63\x74\x00\x6d\x6f\x64\x65\x00\x00\x00\x00\x72\x65\x63\x6f\x76\x65\x72\x00"
	    append replace "\x68\x64\x64\x63\x6f\x70\x79\x00"
	    ...
	    append search "\x7c\xa4\x2b\x78\x7c\xc5\x33\x78\x48\x00\x03\xe1"
            ...
            set search  "\x88\x1c\x01\x2f\x7d\x29\x53\x78\x7c\x00\x4b\x78\xf8\x1f\x01\xb0"
            set replace  "\x38\x00\x00\x1b"
