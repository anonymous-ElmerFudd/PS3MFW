...
namespace eval ::patch_cos {
	...
	proc Do_LV2_Patches {elf} {
		...
		if {$::patch_cos::options(--patch-lv2-payload-hermes-4x)} {
			...
			# OFW 4.50: 0x2F9C48 (0x2E9C48) ** matched to payload from Rogero's CFW 4.50 **
			# HERMES PAYLOAD is currently setup for 4.50 OFW, so it will need to be adjust
			
			# if you are building a CFW from ANY OTHER VERSION, then pop the user box, 
			# and error out, as it will NOT work properly until fixed with the correct offsets
			if {${::NEWMFW_VER} != "4.50"} {
				tk_messageBox -default ok -message "WARNING: HERMES PAYLOAD is currently setup for VER 4.50!!  You need to fix it before continuing!" -icon warning
				die "HERMES PAYLOAD needs to be fixed for your CFW version!"
			}
			log "Patching Hermes payload 4.xx into LV2"				
			#set search     "\x52\x52\x30\x20\x3A\x20\x30\x78"	(old patch)
			set search 		"\x23\x20\x49\x6E\x74\x65\x72\x72\x75\x70\x74\x28\x65\x78\x63\x65"
			...
			append replace  "\x67\xE4\x00\x2E\x60\x84\x9C\xBC\x38\xA0\x00\x02\x4B\xD6\x3A\x0D"
			append replace  "\x28\x23\x00\x00\x40\x82\x00\x28\x67\xFF\x00\x2E\x63\xFF\x9C\xCC"
			append replace  "\xE8\x7F\x00\x00\x28\x23\x00\x00\x41\x82\x00\x14\xE8\x7F\x00\x08"
			append replace  "\x38\x9D\x00\x09\x4B\xD6\x39\x91\xEB\xBF\x00\x00\x7F\xA3\xEB\x78"
			append replace  "\x4B\xFB\x40\x8C\x2F\x61\x70\x70\x5F\x68\x6F\x6D\x65\x00\x00\x00"
			append replace  "\x00\x00\x00\x00\x80\x00\x00\x00\x00\x2E\x9C\xDC\x80\x00\x00\x00"
			append replace  "\x00\x2E\x9C\xEA\x2F\x64\x65\x76\x5F\x66\x6C\x61\x73\x68\x2F\x70"
