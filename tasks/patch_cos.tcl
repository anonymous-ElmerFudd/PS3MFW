...
# Option --patch-appldr-fself-340: [3.xx]  APPLDR: --> Patch Appldr to allow Fself (3.40-3.55) set debug true (Experimental!)
# Option --patch-appldr-fself-330: [3.xx]  APPLDR: --> Patch Appldr to allow Fself (3.10-3.30) set debug true (Experimental!)
# Option --add-356keys-to-appldr341: [3.xx]  APPLDR: --> Patch Appldr to add the 3.56 keys to appldr 3.41
# Option --add-360keys-to-appldr355: [3.xx]  APPLDR: --> Patch Appldr to add the 3.60 keys to appldr 3.55
...
# Option --patch-gameos-hdd-region-size: [ALL]  EMER_INIT: --> Patch Recovery Menu - Create GameOS HDD region smaller than default
# Option --patch-profile-gameos-bootmem-size: [ALL]  DEFAULT.SPP: --> Patch Profile (SPP) - Increase boot memory size of GameOS (Needed for OtherOS++)
# Option --patch-pup-search-in-game-disc: [ALL]  EMER_INIT: --> Patch Recovery Menu - Disable searching for update packages in GAME disc
...
namespace eval ::patch_cos {
	...
	proc Do_LV2_Patches {elf} {
		...
		if {$::patch_cos::options(--patch-lv2-peek-poke-4x)} {
			...
			set ::patch_cos::replace    "\x3B\xE0\x00\x00"
			set ::patch_cos::offset 0
			# base function to decrypt the "self" to "elf" for patching
			::patch_cos::patch_elf $elf			 					
		...
		if {$::patch_cos::options(--patch-lv2-lv1-peek-poke-4x)} {
			...
			set ::patch_cos::search     "\x83\x86\x5C\xCB\x37\x6F\x5D\x5C\x43\x93\xA4\xBA\x53\x35\x90\x03"			
			...
			if {$FWVer >= "370"} {
				set ::patch_cos::offset 2027
			} else {
				set ::patch_cos::offset 1979
			}
			# base function to decrypt the "self" to "elf" for patching
			::patch_cos::patch_elf $elf					   
		}
		...
		if {$::patch_cos::options(--patch-lv2-npdrm-ecdsa-check)} {
			...
			set ::patch_cos::search     "\x41\x9E\xFD\x68\x4B\xFF\xFD\x68\xE9\x22\x99"
		...
		if {$::patch_cos::options(--patch-lv2-SC36-4x)} {
			...
			set ::patch_cos::search     "\x41\x9E\x00\x70\x38\x61\x00\x70\x4B\xFF"
			set ::patch_cos::replace    "\x60\x00\x00\x00"
			set ::patch_cos::offset 0
			# base function to decrypt the "self" to "elf" for patching
			::patch_cos::patch_elf $elf					 
		...
		if {$::patch_cos::options(--patch-lv2-payload-hermes-4x)} {
			...
			append ::patch_cos::replace    	"\x67\xE4\x00\x2E\x60\x84\xAA\xBC\x38\xA0\x00\x02\x4B\xD6\x2C\x11"
			append ::patch_cos::replace    	"\x28\x23\x00\x00\x40\x82\x00\x28\x67\xFF\x00\x2E\x63\xFF\xAA\xCC"
			append ::patch_cos::replace    	"\xE8\x7F\x00\x00\x28\x23\x00\x00\x41\x82\x00\x14\xE8\x7F\x00\x08"
			append ::patch_cos::replace    	"\x38\x9D\x00\x09\x4B\xD6\x2B\x95\xEB\xBF\x00\x00\x7F\xA3\xEB\x78"
			append ::patch_cos::replace    	"\x4B\xFD\x9D\x1C\x2F\x61\x70\x70\x5F\x68\x6F\x6D\x65\x00\x00\x00"
			append ::patch_cos::replace    	"\x00\x00\x00\x00\x80\x00\x00\x00\x00\x2E\xAA\xDC\x80\x00\x00\x00"
			append ::patch_cos::replace    	"\x00\x2E\xAA\xEA\x2F\x64\x65\x76\x5F\x66\x6C\x61\x73\x68\x2F\x70"
			append ::patch_cos::replace    	"\x6B\x67\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"	
			set ::patch_cos::offset 0
			# base function to decrypt the "self" to "elf" for patching
			::patch_cos::patch_elf $elf						
		}		
	...
	proc Do_Misc_OS_Patches {path} {	
		...
		if {$::patch_cos::options(--patch-appldr-fself-340) || $::patch_cos::options(--patch-appldr-fself-330) || $::patch_cos::options(--add-356keys-to-appldr341) || 
		$::patch_cos::options(--add-360keys-to-appldr355)} {												
		...
		if { $::patch_cos::options(--patch-pup-search-in-game-disc) || [expr "${::patch_cos::options(--patch-gameos-hdd-region-size)}" ne ""]} {
			...
			if {$::patch_cos::options(--patch-pup-search-in-game-disc)} {
				...
				set ::patch_cos::search  "\x80\x01\x00\x74\x2F\x80\x00\x00\x40\x9E\x00\x14\x7F\xE3\xFB\x78"
