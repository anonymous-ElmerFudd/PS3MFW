#!/usr/bin/tclsh
#
# RedDot-3ND7355 (Edmayc7, edmayc7@hotmail.fr)
...
namespace eval ::666_990_spoof_version {
    ...
    proc main {} {
      ...
      if {$release != "" || $build != "" || $target != "" || $auth != ""} {
        ...
        ::modify_devflash_file [file join dev_flash vsh etc version.txt] ::666_990_spoof_version::version_txt
      }
      if {$build != "" || $auth != ""} {
        log "Patching vsh.self"
        ::modify_devflash_file [file join dev_flash vsh module vsh.self] ::666_990_spoof_version::patch_self
      }
    }

    proc patch_self {self} {
      ::modify_self_file $self ::666_990_spoof_version::patch_elf
    }
...
#      catch_die {::patch_elf $elf $search 4 $replace} "Unable to patch self [file tail $elf]"
    ...
    proc version_txt {filename} {
      ...
      if {$build != [get_fw_build $filename]} {
        ...
        set build_num $build
        set build_date [lindex [split [lindex [split [::666_990_spoof_version::get_fw_build $filename] ":"] 1] ","] 1]
